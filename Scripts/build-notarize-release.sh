...
RELEASE_DISPLAY_VERSION="${RELEASE_DISPLAY_VERSION:-}"
if [[ -n "$RELEASE_DISPLAY_VERSION" && ! "$RELEASE_DISPLAY_VERSION" =~ ^[0-9]+\.[0-9]+\.[0-9]+(-stg\.[1-9][0-9]*)?$ ]]; then
    echo "유효하지 않은 RELEASE_DISPLAY_VERSION입니다." >&2
    exit 1
fi
...
CLAUDEUSAGE_RELEASE_VERSION = ${RELEASE_DISPLAY_VERSION:-}
EOF

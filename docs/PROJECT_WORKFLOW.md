...
관련된 변경을 한 커밋으로 묶고 dev에 push합니다. 코드 리뷰와 전체 테스트·Release 빌드·실앱 검증을 마친 뒤 main에 squash합니다. staged tree가 검증한 dev tree와 같은지 확인하고, 게시 산출물은 그 main 커밋에서 다시 만듭니다. 다음 작업 전에는 반영이 끝난 dev를 새 main 이력에 정렬합니다. staging은 다음 운영 예정 버전의 검증 회차(`vX.Y.Z-stg.N`)로 구분하며 새 후보마다 독립 build를 증가시킵니다. 운영은 승인 후보를 명시하고 같은 배포 입력과 build를 사용합니다. 완료 보고서 등 일반 문서만 추가된 커밋 때문에 새 운영 버전을 만들지 않습니다. 상세 일치 기준은 [배포 계약](RELEASE.md)을 따릅니다.

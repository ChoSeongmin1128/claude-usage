...
Homebrew 설치 후 메뉴바 아이콘이 보이지 않으면 앱이 실행 중인지 확인하고, 시스템 설정의 메뉴바 표시 상태와 사용 중인 메뉴바 관리 앱의 숨김 설정을 확인한 뒤 Finder에서 다시 실행하세요. 2.6.0에서는 숨김 상태이거나 팝오버 위치를 확인할 수 없을 때 설정창을 엽니다. 화면 좌표만으로 macOS 차단을 단정하지 않습니다.

...
Antigravity의 조회 대상 선택·현재 로그인 확인과 quota 정책은 [Antigravity 사용량 소스와 설정 UX](antigravity-usage-sources.md)를 기준으로 합니다.
...
- 자격이 만료됐거나 확인 실패 상태면 사용자는 `claude auth login`을 다시 실행해 복구할 수 있습니다.
...
Messages 응답 헤더를 이용한 보조 경로는 구형 설정 호환을 위해 내부에 유지합니다. 현재 설정 화면은 이 정책이나 수동 테스트를 노출하지 않으며 신규 설치의 기본값은 `끄기`입니다.

이전 버전에서 저장한 `자동 보조`가 남아 있는 경우에만 OAuth 사용량 조회 실패 뒤 정책과 임계값을 만족할 때 시도합니다. 주 조회 경로나 사용자에게 권장하는 복구 절차로 취급하지 않습니다.

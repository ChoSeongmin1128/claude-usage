...
- 2026-04-02 73차 통합에서 [UpdateService.swift](/Users/seongmin/Personal/ClaudeUsage/ClaudeUsage/Services/UpdateService.swift) 는 `AppUpdateEngine` 추상화 뒤로 현재 GitHub Release 수동 업데이트를 넣기 시작했습니다. 아직 Sparkle 패키지를 프로젝트에 붙인 상태는 아니지만, 업데이트 코드는 이제 `엔진 인터페이스 -> 현재 구현체(GitHub)` 구조를 갖추기 시작했으므로 Sparkle 래퍼를 같은 자리에 넣을 수 있습니다.

...
알림의 ‘알릴 시점’은 표시 방식이 바뀌어도 같은 조건을 유지합니다. 예를 들어 사용 85%는 남음 15%로 표시됩니다. 서비스 행을 펼쳐 실제 조회된 시간·모델별 한도를 알림 대상으로 선택할 수 있습니다. Claude의 모델별 한도와 Codex의 플랜별·추가 모델 한도, Antigravity의 한도를 지원하며 크레딧 잔액·초기화권 개수는 제외합니다. 새로 발견된 한도는 직접 켜야 하며, 표시 숨김과 알림 선택은 독립적입니다.

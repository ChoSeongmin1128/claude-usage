...
설정 → 일반 → 모션에서 즉시·부드러움·사용자 설정을 선택합니다. 사용자 설정에서는 팝오버 열기·닫기와 크기 전환, 설정 탭·빠른 시작 단계, 상세 접기·펼치기, 표시 항목 이동·숨기기, 사용량 막대 갱신을 각각 조정할 수 있습니다. 기본값은 즉시이며 macOS의 ‘동작 줄이기’를 우선합니다. 각 항목의 미리보기에서 두 방식을 비교할 수 있으며, 미리보기는 저장된 설정을 바꾸지 않습니다.

계정 상세와 고급 진단은 제목 행을 눌러 펼칩니다. 계정의 ‘사용’ 버튼은 상세 펼침과 별도로 동작합니다. 디자인 비교와 짧은 선택 항목은 창 폭에 맞춰 가로 또는 세로로 배치됩니다.
